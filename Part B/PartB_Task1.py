...
class Drone(object):
    ...
    def __init__(self, name, nQ, buffer_size, charge_time_threshold=0, MaxAutonomyLevel=1500, PVType="None"):
...
# ******************************************************************************
# Time Converter from Seconds to HOUR,MIN,SEC
# *********************************************************************
...
def arrival(time, drone, my_drones, service_time, inter_arrival):
    ...
    rand_choice = random.randint(0, n_drones-1)
...
#Threshold for going to Charge (Distance Between Charging Station and Drone Area)
charge_time_threshold = 0

durations = [[0, 86400], [28800, 57600], [32400, 54000], [36000, 50400]]

Simulation_Results = []

for time, SIM_TIME in durations:

    time = time
    initial = time
    SIM_TIME = SIM_TIME

    duration = (time, SIM_TIME)
    ...
    for i in range(n_drones):
        My_Drones.append(Drone(name=drone_names[i], nQ=i+1, buffer_size=buffer_size, charge_time_threshold=charge_time_threshold))
    ...
    while time < SIM_TIME:
        ...
        for i in range(n_drones):
            ...
            results = (My_Drones[i].nQ, time, duration, My_Drones[i].n_arrival, My_Drones[i].n_departure, My_Drones[i].loss,
                   My_Drones[i].cycle, My_Drones[i].status, My_Drones[i].charge_level)
            My_Drones[i].log.append(results)
    
    Simulation_Results.append(My_Drones[0].log)
    ...
    # ******************************************************************************
    print('\n ========= STATS - Task 1 =========')
    print("\n Number of arrivals:", My_Drones[0].n_arrival)
    print("\n Number of transmitted packets:", My_Drones[0].n_departure)
    print("\n Number of dropped packets:", My_Drones[0].loss)
    print("\n Average number of packets:", round(My_Drones[0].ut/time, 3)) # obtained E[N]\
    print("\n Expected average number of packets:", round(rho/(1-rho), 3)) # expected E[N]
    print("\n Average system delay:", round(My_Drones[0].delay/My_Drones[0].n_departure, 3) ,"ms") # obtained E[T]
    print("\n Expected average system delay:", round(1/(mu - lambtda),3) ,"ms") # expected E[T]
    print("\n Average queue delay:", round(((My_Drones[0].delay - My_Drones[0].st)/My_Drones[0].n_departure),3),"ms" ) #Obtained E[Tw]
    print("\n Expected average queue delay:", round(rho / (mu - lambtda),3),"ms") # expected E[Tw]
    print("\n Average number of packets in queue:", round(My_Drones[0].utq / time,3)) #obtained E[Nw] 
    print("\n Expected average number of packets in queue:", round((rho * rho) / (1 - rho),3)) #expected E[Nw]
    print("\n Loss probability:",round((My_Drones[0].loss/My_Drones[0].n_arrival)*100,3),"%")
    print("\n Busy time:", round(My_Drones[0].st, 3),"%")
    print("\n Expected busy time:", round(rho * 100, 3),"%")
    print("\n Number of Cycles:", My_Drones[0].cycle)
    print("\n Remaining Charge:", My_Drones[0].charge_level)
    print("\n")
    ...
    fig = plt.figure()
    sns.histplot(data=My_Drones[0].delays, stat='count', kde=True, color='blue')
    plt.xlabel("Queue Delay (ms)")
    plt.ylabel("Frequency")
    plt.title(f"Distribution of the Queuing Delay Between {hours_t} and {hours_s} Hours")
    plt.xlim(0.1000)
    plt.grid()
    plt.savefig(f"dist_queing_delay_{hours_t}_{hours_s}.png", dpi=300)
    plt.show()
...
for simulation in Simulation_Results:
    ...
    fr_h, fr_m, fr_s = time_converter(simulation[0][2][0], mode='num')
    to_h, to_m, to_s = time_converter(simulation[0][2][1], mode='num')
