...
class Drone(object):
    ...
    def __init__(self, name, nQ, buffer_size, drone_type, charge_time_threshold=0, MaxAutonomyLevel=1500, service_rate=5):
        ...
        elif self.drone_type=="TYPE_B":
            self.service_rate = service_rate/2
            self.buffersize = buffer_size
            self.charge_increase=300 
        elif self.drone_type=="TYPE_C":
            self.service_rate = service_rate
            self.buffersize = 2*buffer_size
            self.charge_increase=900
            
        #Charge Staff
        self.cycle = 0
        self.charged_seconds = 0
        self.charge_level = 1500 + self.charge_increase
        self.max_autonomy_level = MaxAutonomyLevel + self.charge_increase
        self.status = "Online"
...
My_Drones.append(Drone(name=drone_names[0], nQ=1, buffer_size=buffer_size, drone_type="TYPE_C", charge_time_threshold=charge_time_threshold, service_rate=10))
My_Drones.append(Drone(name=drone_names[1], nQ=2, buffer_size=buffer_size, drone_type="TYPE_C", charge_time_threshold=charge_time_threshold, service_rate=10))
...
while time < SIM_TIME:
    ...
    for i in range(len(My_Drones)):
        if len(My_Drones[i].FES.queue):
            ...
            SERVICE = My_Drones[i].service_rate # Average Service Time
        ...
        results = (My_Drones[i].nQ, time, My_Drones[i].n_arrival, My_Drones[i].n_departure, My_Drones[i].loss,
