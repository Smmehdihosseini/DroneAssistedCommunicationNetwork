...
drone_joined=0
while time < SIM_TIME:
    if time >= 37800 and drone_joined==0:
        My_Drones.append(Drone(name=drone_names[1], nQ=2, buffer_size=buffer_size, charge_time_threshold=charge_time_threshold))
        My_Drones[1].FES.put((time, "Arrival"))
        drone_joined+=1
        print(f"It's {My_Drones[0].name}. I'm going to take a rest right now! Good Luck {My_Drones[1].name}")
        My_Drones[0].status = "Hibernate"
        My_Drones[0].service_condition == "Hibernate"
        while My_Drones[0].users>0:
            print(f"B-DEP Drone {str(My_Drones[0].nQ)} - {My_Drones[i].name} {My_Drones[i].status} ({My_Drones[i].charge_level}): Departure Canceled at time {time_converter(time, mode='print')} with {My_Drones[i].users-1} users in queue!")
            client = My_Drones[0].queue.pop(0) 
            My_Drones[0].in_service -= 1
            My_Drones[0].users -= 1
            data.users -= 1
            My_Drones[0].loss += 1
            My_Drones[0].loss_outofactivity += 1
            
    if time >= 48600 and drone_joined==1:
        My_Drones.append(Drone(name=drone_names[2], nQ=3, buffer_size=buffer_size, charge_time_threshold=charge_time_threshold))
        My_Drones[2].FES.put((time, "Arrival"))
        drone_joined+=1
        print(f"It's {My_Drones[1].name}. I'm going to take a rest right now! Good Luck {My_Drones[2].name}")
        My_Drones[1].status = "Hibernate"
        My_Drones[1].service_condition == "Hibernate"
        while My_Drones[0].users>0:
            print(f"B-DEP Drone {str(My_Drones[0].nQ)} - {My_Drones[i].name} {My_Drones[i].status} ({My_Drones[i].charge_level}): Departure Canceled at time {time_converter(time, mode='print')} with {My_Drones[i].users-1} users in queue!")
            client = My_Drones[0].queue.pop(0) 
            My_Drones[0].in_service -= 1
    ...
    for i in range(len(My_Drones)):
        if len(My_Drones[i].FES.queue) and (My_Drones[i].status!="Hibernate"):
            ...
            if event_type == "Arrival":
                ...
                My_Drones[i].cycle += 1
                while My_Drones[rand_choice].status!="Online":
                    rand_choice = random.randint(0, len(My_Drones))
                My_Drones[rand_choice].FES.put((time + inter_arrival, "Arrival"))
